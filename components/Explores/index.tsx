...
const Explores = () => {
  ...
  const [filteredLocations, setFilteredLocations] = useState(exploreData);
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
            ...
            <div className="flex space-x-2">
              {categories.map((category) => (
                <button
                  key={category.id}
                  className={`px-4 py-2 text-sm font-medium rounded-md ${activeCategory === category.id
              ...
              {filteredLocations.map((location) => (
                <div key={location.id} className="bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300">
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-gray-600 text-sm mb-4">{location.resume_description}</p>
                    <Link href={location.path} className="inline-block bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-medium py-2 px-4 rounded transition-colors duration-200 text-sm">
                      En savoir plus
                    </Link>
                  </div>
                </div>
